...
import { TrendingUp, AlertTriangle, CheckCircle, Activity, DollarSign, Users, Map, Cpu, Zap, Droplet, Truck, Shield, X, MapPin, Clock, Navigation } from 'lucide-react';
...
const OfficialDashboard = () => {
    ...
    // Mock Data for Map Zones
    const mapZones = [
        { id: 1, name: "Central Bridge", type: "Infrastructure", health: 65, status: "WARNING", lat: 40.7128, lng: -74.0060, param: "Vibration" },
        { id: 2, name: "Power Grid A", type: "Energy", health: 98, status: "OPTIMAL", lat: 40.7308, lng: -73.9973, param: "Voltage" },
        { id: 3, name: "Water Main", type: "Utility", health: 92, status: "GOOD", lat: 40.7580, lng: -73.9855, param: "Pressure" },
    ];
    ...
    return (
        <div className="min-h-screen bg-[#05050a] pb-12 font-sans relative">
            <Navbar />

            {/* NEW: Incident Alert Modal */}
            {alert && (
            ...
            <div className="container mx-auto px-6 pt-24 pb-12">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="px-2 py-1 bg-white/5 border border-white/10 text-gray-400 text-[10px] font-bold rounded uppercase tracking-wider flex items-center gap-1">
                                <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></div> Live Data
                            </span>
                        </div>
                        <h1 className="text-3xl font-bold text-white mb-1">Executive Overview</h1>
                        <p className="text-gray-400 text-sm">City Infrastructure Health & Performance | Zone A-1</p>
                    </div>
                    <div className="flex gap-3">
                        <button className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-gray-300 hover:bg-white/10 transition-colors flex items-center gap-2">
                            <Activity size={16} /> Generate Report
                        </button>
                        <button
                            onClick={() => setShowScanner(true)}
                            className="px-4 py-2 bg-[#00FFA3]/20 border border-[#00FFA3]/50 rounded-lg text-sm text-[#00FFA3] hover:bg-[#00FFA3]/30 transition-colors shadow-[0_0_15px_rgba(0,255,163,0.15)] flex items-center gap-2"
                        >
                            <Scan size={16} /> AI Vision Analysis
                        </button>
                    </div>
                </div>
                ...
                {/* Structural Health & Digital Twin Section */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-[600px]">
                    ...
                    <div className="lg:col-span-2 glass-panel p-0 relative overflow-hidden group h-full">
                        ...
                        <div className="absolute bottom-6 left-6 pointer-events-none z-[1000]">
                            <h3 className="text-xl font-bold text-white mb-1">Spatial Digital Twin</h3>
                            <p className="text-sm text-gray-400">Interactive Infrastructure Map • Zone A-1</p>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-col h-full animate-fade-in">
                                <div className="p-6 border-b border-white/5 bg-white/5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className={`px-2 py-0.5 rounded text-[10px] font-bold border ${selectedZone.status === 'WARNING' ? 'border-red-500 text-red-500 bg-red-500/10' : 'border-green-500 text-green-500 bg-green-500/10'}`}>
                                            {selectedZone.status}
                                        </div>
                                ...
                                <div className="p-6 flex-1 overflow-y-auto">
                                    ...
                                    <div className="mb-6">
                                        <div className="flex justify-between items-end mb-2">
                                            <span className="text-xs text-gray-400 uppercase tracking-wider">{selectedZone.param} Stability</span>
                                            <span
                                                className={`text-lg font-mono font-bold transition-colors duration-200 ${currentStability < 50 ? 'text-red-500 animate-pulse' :
                                        ...
                                        <VirtualSensorGraph
                                            isActive={true}
                                            color={selectedZone.status === 'WARNING' ? '#EF4444' : '#00D1FF'}
                            ...
                            {riskData.map((item, index) => (
                                <div key={index} className="space-y-1">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className={`${item.riskScore > 80 ? 'text-red-500' : item.riskScore > 50 ? 'text-yellow-500' : 'text-green-500'} font-bold`}>
                                            {item.riskScore > 80 ? 'CRITICAL' : item.riskScore > 50 ? 'WARNING' : 'STABLE'}
                                        </span>
                                    </div>
                                    <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                                        <div
                                            ...
                                            style={{
                                                ...
                                                backgroundColor: item.riskScore > 80 ? '#FF4D4D' : item.riskScore > 50 ? '#FFBD00' : '#00FFA3'
                                            }}
                                        ></div>
                                    </div>
                                </div>
